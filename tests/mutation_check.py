...
MUTANTS = [
    ...
    ("parallel gear: segment warm-up one byte short", "src/parallel.cpp",
     "constexpr std::size_t kBack = detail::kGearWindow - 1;", "constexpr std::size_t kBack = detail::kGearWindow - 2;", "kill"),
    ("parallel rabin: segment warm-up one byte short", "src/parallel.cpp",
     "constexpr std::size_t kBack = detail::kRabinWindow - 1;", "constexpr std::size_t kBack = detail::kRabinWindow - 2;", "kill"),
    ("parallel: strict mask ignored below the average", "src/parallel.cpp",
     "if (!gear || length >= p.avg_size || cands[k].strict) {", "if (true) {", "kill"),
    ("parallel: first usable candidate one byte late", "src/parallel.cpp",
     "cands[next].pos < start + p.min_size - 1", "cands[next].pos < start + p.min_size", "kill"),
    ("parallel: candidates past max_size accepted", "src/parallel.cpp",
     "if (length > limit) break;", "if (length > limit + 1) break;", "kill"),
